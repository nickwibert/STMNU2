...
class StudentInfoWindow(ctk.CTkToplevel):
    ...
    def create_search_results_window(self):
        ...
        # Search for matches
        self.matches = self.database.search(query)
        self.geometry(f'{100*self.matches.shape[1]}x500')
        self.rowconfigure(0, weight=1)
        self.columnconfigure(0, weight=1)
    ...
    def create_student_info_window(self, student_idx):
        ...
        # Populate student info frame with information
        self.create_student_info_labels()
        self.student_info_frame.columnconfigure(0, weight=1)
        self.student_info_frame.rowconfigure(0,weight=1)
        self.student_info_frame.grid(row=2, column=0)
        self.student_info_frame.rowconfigure(tuple(i for i in range(1,10)), weight=1)

        # Frame which will contain class information
        self.class_info_frame = MyFrame(self.main_frame)
    ...
    def create_student_info_labels(self):
        ...
        self.student_info = self.database.student.iloc[self.student_idx].astype('string').fillna('').str.title()

        # Display student number above name (cannot be changed)
        self.studentno_label = ctk.CTkLabel(self.student_info_frame, text=f'#{self.student_info['STUDENTNO']}',
                                            font=ctk.CTkFont('Britannic',18))
        self.studentno_label.grid(row=0,column=0,sticky='nsew')
        ...
        self.address_frame.grid(row=2,column=0,rowspan=2, sticky='nsew')
        ...
        # Email address
        self.labels['EMAIL'] = ctk.CTkLabel(self.student_info_frame, text=self.student_info['EMAIL'])
        self.labels['EMAIL'].grid(row=4, column=0, sticky='nsew')

        # Create frame for mother name
        self.mom_frame = MyFrame(self.student_info_frame)
        self.mom_frame.columnconfigure((0,1), weight=1)
        self.mom_frame.grid(row=5,column=0, sticky='nsew')
        ...
        self.dad_frame.grid(row=6,column=0, sticky='nsew')
        ...
        self.enrolldate_frame.grid(row=9,column=0, sticky='nsew')
        ...
        self.monthlyfee_frame.grid(row=10,column=0, sticky='nsew')
        ...
        self.balance_frame.grid(row=11,column=0, sticky='nsew')
    ...
    def confirm_edit(self):
        ...
        for field in self.entry_boxes.keys():
            field_info = self.database.dbf_table.field_info(field)
    ...
    def go_prev_student(self):
        ...
        student_sorted_idx = self.database.sort_alphabetical().index.get_loc(self.student_idx)
        # If current student is the first student alphabetically, there are no previous students
        if student_sorted_idx == 0:
            return
        else:
            prev_student_idx = self.database.sort_alphabetical().index[student_sorted_idx - 1]
    ...
    def go_next_student(self):
        ...
        if student_sorted_idx == (self.database.student.shape[0]-1):
            ...
            next_student_idx = self.database.sort_alphabetical().index[student_sorted_idx + 1]
