...
### New Class Dialog ###
# This pop-up window is used when the user requests to create a new class
class NewClassDialog(DialogBox):
    def __init__(self, database, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.database = database
        self.dropdowns = {}

        self.after(200, lambda : self.entry_boxes['CLASSTIME'].focus())
    ...
    def _create_widgets(self):
        ...
        self.class_frame.columnconfigure((0,1),weight=1)
        self.class_frame.grid(row=0,column=0,sticky='nsew')

        # Get list of all instructors from database
        self.database.cursor.execute('SELECT DISTINCT TEACH FROM classes ORDER BY TEACH')
        instructor_names=[teach[0].title() for teach in self.database.cursor.fetchall()]
        self.dropdowns['TEACH'] = ctk.CTkOptionMenu(self.class_frame, values=instructor_names, width=150)
        self.dropdowns['TEACH'].grid(row=0, column=0, columnspan=2)

        day_names = [day for day in calendar.day_name[:6]]
        self.dropdowns['WEEKDAY'] = ctk.CTkOptionMenu(self.class_frame, values=day_names, width=150)
        self.dropdowns['WEEKDAY'].grid(row=1, column=0, columnspan=2)

        class_field_position = {
            'CLASSTIME'      : {'row' : 2, 'column' : 0, 'columnspan' : 1},
            'MAX'       : {'row' : 2, 'column' : 1, 'columnspan' : 1},
            'CLASSNAME' : {'row' : 3, 'column' : 0, 'columnspan' : 2},
        }

        self.entry_boxes = {}
        for field, kwargs in class_field_position.items():
            entry = ctk.CTkEntry(master=self.class_frame, placeholder_text=field, font=ctk.CTkFont('Segoe UI',20))
            if field == 'CLASSTIME':
                entry.dtype = 'time'
            elif field == 'MAX':
                entry.dtype = 'float'
        ...
        self.wait_var.set('validate')
        # Button to confirm info
        self.confirm_button = ctk.CTkButton(self, text="Create Class")
        self.confirm_button.configure(command=lambda c=self.confirm_button, eb=self.entry_boxes,
                                                     ef=self.error_frame, v=self.wait_var:
                                                        fn.validate_entryboxes(c, eb, ef, v))
        # Store confirm command to re-assign it to button later
        ...
        while self.wait_var.get() != 'close':
            if self.wait_var.get() == 'exit':
                self._wait_event()
                ctk.CTkLabel(self.error_frame, width=100,
                             text='Exit without creating class?',
                             text_color='red').grid(row=0,column=0)
                ctk.CTkButton(self.error_frame, text='Yes', command=self._exit_event).grid(row=1,column=0)
                ctk.CTkButton(self.error_frame, text='No', command=self._wait_event).grid(row=2,column=0)
                self.confirm_button.configure(state='disabled')
            else:
                # If we get here, the data entry has finally been validated and confirmed, so we create the class
                # create dummy entry boxes with instructor / day of week 
                for field in ['TEACH', 'WEEKDAY']:
                    entry = ctk.CTkEntry(
                    ...
                    entry.dtype = 'string'
                    # Insert text from dropdown selection
                    entry.insert(0, self.dropdowns[field].get())
                    self.entry_boxes[field] = entry

                # send all class info to create class function
                self.database.create_class(self.entry_boxes)
                break
